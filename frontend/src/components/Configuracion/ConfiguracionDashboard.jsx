...
const ConfiguracionDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      const response = await fetch('/api/configuracion/dashboard/', {
        credentials: 'include',
      });
      if (response.ok) {
        const data = await response.json();
        setStats(data.stats);
        setConfigGeneral(data.config_general);
        setLogsRecientes(data.logs_recientes);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-purple-600 to-indigo-600 text-white p-6 rounded-lg shadow-lg">
        ...
        <p className="mt-2 opacity-90">Panel de control de configuraciones generales</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-semibold text-gray-900">{stats.total_parametros}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              <p className="text-sm font-medium text-gray-600">Parámetros Activos</p>
              <p className="text-2xl font-semibold text-gray-900">{stats.parametros_activos}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-semibold text-gray-900">{stats.modulos_activos}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-semibold text-gray-900">{stats.logs_hoy}</p>
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="space-y-3">
            ...
            >
              <div className="flex items-center">
                ...
                <span className="font-medium">Gestión de Módulos</span>
              </div>
              <svg className="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5l7 7-7 7" />
              </svg>
            ...
            <button
              onClick={() => navigate('/configuracion/logs')}
              className="w-full flex items-center justify-between p-3 text-left border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors"
            >
              <div className="flex items-center">
              ...
              logsRecientes.map((log, index) => (
                <div key={index} className="flex items-start space-x-3 p-3 bg-gray-50 rounded-lg">
                  <div className={`p-1 rounded-full ${
                    log.nivel === 'error' ? 'bg-red-100 text-red-600' :
                    log.nivel === 'warning' ? 'bg-yellow-100 text-yellow-600' :
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-gray-900 truncate">{log.descripcion}</p>
                    <p className="text-xs text-gray-500">{log.usuario} - {log.fecha_creacion}</p>
                  </div>
                </div>
              ))
            ) : (
              <p className="text-gray-500 text-center py-4">No hay logs recientes</p>
            )}
          </div>
        </div>
      </div>
