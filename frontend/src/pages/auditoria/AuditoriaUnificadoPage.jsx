...
const AuditoriaUnificadoPage = () => {
  ...
  useProductTour('auditoria', TOUR_CONFIGS.auditoria.steps, {
    ready: true,
  })

  // Permiso de página completa
  if (!hasPermission('auditoria.view')) return <div className="p-8 text-center text-red-500 font-semibold">No tienes permisos para acceder a esta sección</div>
  ...
  const tabs = allTabs.filter(tab => hasPermission(tab.permiso))
  ...
  return (
    <div className="p-8 space-y-8">
      {/* Header */}
      <div id="tour-auditoria-header" className="backdrop-blur-xl bg-gradient-to-r from-blue-500 to-purple-600 rounded-3xl shadow-2xl p-8 text-white">
      ...
      {/* Tabs Navigation */}
      <div id="tour-auditoria-tabs" className="backdrop-blur-xl bg-white/90 rounded-2xl shadow-lg p-2 border border-gray-200/50">
      ...
      {/* Tab Content */}
      <div id="tour-auditoria-content" className="animate-fadeIn">
        {renderTabContent()}
      </div>
    </div>
  )
